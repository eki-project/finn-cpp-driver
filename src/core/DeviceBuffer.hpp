...
namespace Finn {
    ...
    class RingBufferAssignmentProxy {
        ...
        /**
         * @brief Handle assignment by setting the ring buffer values to the values from the passed array. Note that this does not check for size, so it's the user's responsibility. The array must be sized as large as size(SIZE_SPECIFIER::ELEMENTS) 
         * 
         * @param arr 
         * @return RingBufferAssignmentProxy& 
         */
        RingBufferAssignmentProxy& operator=(const T& arr) {
            for (unsigned int i = 0; i < elementsPerPart; i++) {
                ringBuffer[(elementIndex + i) % ringBuffer.size()] = arr[i];
            }
            return *this;
        }
        ...
        std::array<T,S> unpackToArray() {
            if (S < elementsPerPart) {
                FinnUtils::logAndError<std::length_error>("Tried to unpack " + std::to_string(elementsPerPart) + " elements into an array of size " + std::to_string(S));
            }
            std::array<T,S> temp;
            for (unsigned int i = 0; i < elementsPerPart; i++) {
    ...
    class DeviceBuffer {
        ...
        // TODO: REFACTOR METHODS FOR BOTH USER VARIANTS AS DISCUSSED 


        friend std::ostream& operator<<(std::ostream& stream, const DeviceBuffer& buffer) {
            std::string tempShapeStr(buffer.bufferShape.begin(), buffer.bufferShape.end());
            stream << magic_enum::enum_name(buffer.bufferIOMode) << " DeviceBuffer " <<  buffer.name << " (Shape: " << tempShapeStr << ", Numbers: " << buffer.sizeNumbers << ", Elements: " << buffer.sizeElements << ", Bytes: " << buffer.sizeBytes << " Ring Buffer Elements: " << buffer.ringBufferElementSize << ")";
        }

        /**
         * @brief When used, this loads the data into the ring buffer, but does NOT load it into the map 
         * 
         * @param stream 
         * @param buffer 
         * @return std::istream& 
         */
        friend std::istream& operator>>(std::istream& stream, DeviceBuffer& buffer) {
            // TODO: Change for consistency? operator<< shows a string rep, operator>> loads data?
        ...
        void set(const unsigned int partIndex, const T& arr, const unsigned int arrSize, bool makeCurrent = true) {
            ...
            if (partIndex >= ringBufferActiveParts) {
                ...
                //  NOLINTNEXTLINE
                FinnUtils::logAndError<std::length_error>("Trying to assign a DeviceBuffer at index " + std::to_string(partIndex) + " but the max index is " + std::to_string(ringBufferActiveParts));
            }
            if (arrSize != sizeElements) {
                // clang-tidy warnings in this line are false positive: https://github.com/llvm/llvm-project/issues/57959
                // NOLINTNEXTLINE
                FinnUtils::logAndError<std::length_error>("Mismatch in sizes: Trying to assign a ring buffer active part with an array of size " + std::to_string(arrSize) + " but a ring buffer active part has " + std::to_string(sizeElements) + " elements!");
            }
            ...
            if (makeCurrent) {
                ringBufferElementIndex = (partIndex * sizeElements) % ringBufferElementSize;
            }
        }

        /**
         * @brief Append values from an array to the ring buffer parts. At the end, the internal pointer points to the BEGINNING of the part. 
         * 
         * @param arr 
         * @param arrSize 
         */
        void append(const T& arr, const size_t arrSize) {
            set(getCurrentIndex(), arr, arrSize, true);
        }
