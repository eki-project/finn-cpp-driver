...
namespace FinnUtils {
    ...
    inline unsigned int innermostDimension(const Finn::shape_t& shape) { return shape.back(); }
    ...
    inline size_t shapeToElements(const Finn::shape_t& pShape) { return static_cast<size_t>(std::accumulate(pShape.begin(), pShape.end(), 1, std::multiplies<>())); }
    ...
    inline std::string shapeToString(const Finn::shape_t& pShape) {
        std::string str = "(";
