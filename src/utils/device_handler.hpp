...
class DeviceHandler {
     ...
     */
    DeviceHandler(const int pDeviceIndex, const std::string& pBinaryFile, const DRIVER_MODE pDriverMode, const Bytewidths& pInputBytewidths, const Bytewidths& pOutputBytewidths, const Shapes& pInputShapes, const Shapes& pOutputShapes,
                  const SHAPE_TYPE pInputShapeType, const SHAPE_TYPE pOutputShapeType)
        : deviceIndex(pDeviceIndex), binaryFile(pBinaryFile), driverMode(pDriverMode) {
    ...
    static std::vector<xrt::bo> createIOBuffers(const xrt::device& device, const Bytewidths& widths, const Shapes& shape) {
        if (widths.size() != shape.size()) {
            throw std::length_error("The number of bytewidths passed does not match the number of shape lists passed (" + std::to_string(widths.size()) + ", " + std::to_string(shape.size()) + ")!");
        }
