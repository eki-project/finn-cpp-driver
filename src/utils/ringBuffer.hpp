...
class RingBuffer {
     ...
     */
    RingBuffer([[maybe_unused]] const unsigned int pSize, [[maybe_unused]] const unsigned int pTargetSize) /*: size(pSize), targetSize(pTargetSize)*/ {
        // data = std::make_unique<T[]>(pSize);
        // currentIndex = 0;
    }

    RingBuffer([[maybe_unused]] const std::vector<T>& elements, [[maybe_unused]] const unsigned int pTargetSize) /*: targetSize(pTargetSize)*/ {
        // size = elements.size();
    ...
    RingBuffer([[maybe_unused]] const T* arr, [[maybe_unused]] const unsigned int pSize, [[maybe_unused]] const unsigned int pTargetSize) /*: targetSize(pTargetSize)*/ {
        // data = std::make_unique<T[]>(pSize);
